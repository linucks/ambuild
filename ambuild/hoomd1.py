...
class Hoomd1(FFIELD):
        ...
        **kw
        ...
        for i in range(retries_on_error):
            ...
            try:
                ...
                if rigidBody:
                    fire = integrate.mode_minimize_rigid_fire(
                        group=self.groupActive,
                        dt=dt,
                        Nmin=Nmin,
                    ...
                    fire = integrate.mode_minimize_fire(
                        group=self.groupActive,
                        dt=dt,
                        Nmin=Nmin,
                        alpha_start=alpha_start,
    ...
    def _runMD(
        ...
        **kw
        ...
        if integrator == "nvt":
            ...
            if rigidBody:
                integ = integrate.npt_rigid(
                    group=self.groupActive, T=T, tau=tau, P=P, tauP=tauP
                )
            else:
                integ = integrate.npt(
                    group=self.groupActive, T=T, tau=tau, P=P, tauP=tauP
    ...
    def setAngle(self, anglePotential):
        for angle in self.angles:
            ...
            if self.debug:
                logger.info(
                    "DEBUG: angle set_coeff( '{0}',  k={1}, t0={2} )".format(
    ...
    def setBond(self, bondPotential):
        for bond in self.bonds:
            ...
            if self.debug:
                logger.info(
                    "DEBUG: bond_coeff.set( '{0}',  k={1}, r0={2} )".format(
    ...
    def setDihedral(self, dihedralPotential):
        for dihedral in self.dihedrals:
            ...
            if self.debug:
                logger.info(
                    "DEBUG: dihedral set_coeff.('{0}',  k={1}, d={2}, n={3})".format(
                        dihedral, param["k"], param["d"], param["n"]
                    )
                )
            dihedralPotential.set_coeff(
        ...
        for i, atype in enumerate(self.atomTypes):
            for j, btype in enumerate(self.atomTypes):
                if j >= i:
                    ...
                    pairPotential.pair_coeff.set(
                        atype, btype, epsilon=epsilon, sigma=sigma
                    )
                    if self.debug:
                        logger.info(
                            "DEBUG: pair_coeff.set( '{0}', '{1}', epsilon={2}, sigma={3} )".format(
    ...
    def setupGroups(self, data):
        ...
        if any(data.static):
            self.groupStatic = group.tag_list(
                name="static", tags=[i for i, s in enumerate(data.static) if s]
            )
            self.groupActive = group.difference(
    ...
    def setupSystem(
        ...
        if rigidBody:
            globals.neighbor_list.reset_exclusions(
                exclusions=["1-2", "1-3", "1-4", "angle", "body"]
            )
        else:
            globals.neighbor_list.reset_exclusions(
                exclusions=["1-2", "1-3", "1-4", "angle"]
            )
            ...
            if do:
                ...
                if not wallstructure:
                    ...
                    for atype in self.atomTypes:
                        ...
                        lj.force_coeff.set(
                            atype, epsilon=param["epsilon"], sigma=param["sigma"]
                        )
        ...
        if atomCount != len(self.system.particles):
            raise RuntimeError(
                "Read {0} positions but there were {1} particles!".format(
                    atomCount, len(self.system.particles)
                )
            )
        ...
        for p in system.particles:
            ...
            if unwrap:
                coord = xyz_core.unWrapCoord3(
                    p.position, p.image, system.box, centered=False
                )
            else:
                pass
    ...
    def writeXml(
        ...
        for i in range(len(d.coords)):
            image += "{0} {1} {2}\n".format(
                d.images[i][0], d.images[i][1], d.images[i][2]
            )
            position += "{0} {1} {2}\n".format(
                d.coords[i][0], d.coords[i][1], d.coords[i][2]
            )
        ...
        if len(d.propers):
            ...
            for i, dh in enumerate(d.propers):
                dihedral += "{0} {1} {2} {3} {4}\n".format(
                    d.properLabels[i], dh[0], dh[1], dh[2], dh[3]
                )
        ...
        ET.SubElement(
            config, "box", Lx=str(d.cell[0]), Ly=str(d.cell[1]), Lz=str(d.cell[2])
        )
...
def xml2xyz(xmlFilename, xyzFilename):
    ...
    positions = [
        line.strip().split() for line in ptext.split(os.linesep) if line.strip()
    ]
