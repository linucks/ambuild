...
class subUnit(object):
    ...
    def __init__(
        ...
        self.tally = [0] * len(
            monomers
        )  # Tracks how many monomers are present in this subunit
        self.totalTally = (
            totalTally  # Tracks how many monomers are present in the overall polymer
        )
        self.random = random
        self.polymer = polymer
        if fragment is not None:
            self.incrementMonomer(
                fragment
    ...
    def incrementMonomer(self, fragment):
        ...
        if not self.random:
            ...
            assert (
                self.tally[self.index] <= self.ratio[self.index]
